...
# Check for bypass keywords in commit message or environment variables
COMMIT_MSG=""
BYPASS_DETECTED=false

# Method 1: Check environment variable directly (for manual bypass)
if [ -n "${SCOPE_BYPASS:-}" ]; then
    echo -e "${GREEN}✅ Scope check bypassed: SCOPE_BYPASS environment variable set${NC}"
    exit 0
fi
...
if [ -n "$COMMIT_MSG" ]; then
    ...
    echo -e "${BLUE}💡 Alternative: Use 'SCOPE_BYPASS=1 git commit -m \"your message\"' to bypass scope protection${NC}"
fi
